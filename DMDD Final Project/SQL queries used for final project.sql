...
--  select * from barnes b  INNER JOIN ebay e on b.prod_name = e.prod_name order by e.price desc;
-- select count(*)from apple_product_price
...
-- drop table barensfinal;
-- select * from dffinal;
-- ALTER TABLE barenfinal
-- DROP COLUMN volume_cloumn2;
-- select * from dffinal;
-- select * from amazon e inner join walmartdata w on e.Product_name = w.Product_name where e.Category = "Electronics|Charger";

 -- select * from barenfinal;
-- ALTER TABLE barenfinal
-- RENAME COLUMN column1 TO volume;

-- select * from amazon_product;

-- drop table amazon_product;
-- select * from walmart_product

-- select Product_Name, Sale_Price from walmartdata group by category order by Sale_Price desc LIMIT 1 ;
-- select * from appleproductincomegroup
 -- select * from appleproductincomegroup
 select * from walmart_product;
...
-- inner join  ebay e on  w.Product_name = e.prod_name where w.Category = "Office Supplies | Paper | All Paper & Printable Media"
-- select e.prod_name,e.price,w.Sale_Price from ebay e inner join walmartdata w on e.prod_name = w.Product_name order by e.price asc 
-- select * from walmartdata w INNER JOIN amazon a ON  w.Product_Name = a.Product_Name group by w.Product_Name order by Sale_Price asc;